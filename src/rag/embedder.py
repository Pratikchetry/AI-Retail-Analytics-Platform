...
class LocalTextEmbedder:
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
        # CRITICAL FIX: The correct endpoint for embeddings is /pipeline/feature-extraction/
    ...
    def encode_text(self, text: str) -> List[float]:
        ...
        # The feature-extraction pipeline returns a list of lists (token embeddings). 
        # We need to mean-pool them to get a single sentence vector.
        if isinstance(data, list) and len(data) > 0:
            # If it's a list of tokens, average them
    ...
    def encode_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        for text in texts:
            ...
            if isinstance(data, list) and len(data) > 0:
                if isinstance(data[0], list):
                    import numpy as np
                    results.append(np.mean(data, axis=0).tolist())
                else:
                    results.append(data)
            else:
                results.append(data)
